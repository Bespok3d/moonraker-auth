# moonraker-auth: instructions for AI assistants
...
This repo publishes one plugin: `moonraker-auth`, which requires a login for the web UI, or allows
open access. It is a solo Bespok3d plugin repo: it ships this single plugin and publishes one index
atom into `Bespok3d/main-index/atoms/`. It ships config, patches, assets, and shell only, no Python.
